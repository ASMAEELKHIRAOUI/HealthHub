            ...
            @foreach ($carts as $cart)
                <div class="d-flex row mb-3">
                    ...
                    <div class="col-2 d-flex justify-content-center h-50">
                        <input type="number" value="{{ $cart->quantity }}" class="rounded mt-3 ps-3" onchange="changeQuantity(event,{{ $cart->id }})" min="1" max="{{ $cart->product->stock }}">
                    </div>
                    <div class="col-2 fs-4 d-flex justify-content-center mt-4">{{ ($cart->product->price - (($cart->product->promotion * $cart->product->price)/100)) * $cart->quantity }}$</div>
